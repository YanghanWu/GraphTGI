...
def Train(directory, epochs, aggregator, embedding_size, layers, dropout, slope, lr, wd, random_seed, ctx):
    ...
    g, TFgene_ids_invmap, NTFgene_ids_invmap,TFSM = build_graph(directory, random_seed=random_seed, ctx=ctx)
    
    samples = sample(directory, random_seed=random_seed)

    samples_df = pd.DataFrame(samples, columns=['TFgene', 'NTFgene', 'label'])
    ...
    for i in range(len(train_index)):
        ...
        g.edges[sample_TFgene_vertices, sample_NTFgene_vertices].data.update(edge_data)
        g.edges[sample_NTFgene_vertices, sample_TFgene_vertices].data.update(edge_data)
